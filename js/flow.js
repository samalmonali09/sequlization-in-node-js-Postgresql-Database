...
app.get('/read', function (req, res) {
  var xml_string = fs.readFileSync("IR.xml", "utf8");
...
app.get('/read-xsd', function (req, res) {
  ...
  validator.validateXML(xmlStr, 'IR.xsd', function (err, result) {
    if (err === null) {
      ...
      request(post_options, function (err, resp, body) {
        ...
        var data = req.body;
        var sqmsver_shipping_information = JSON.parse(JSON.stringify(data.sqmsver_shipping_information));
        var sqmserver = sqmsver_shipping_information.supplier_adress;
        ...
        if (sqmsver_shipping_information) {
          ...
          var reportNum = sqmsver_shipping_information.report_number;
          var create_date = sqmsver_shipping_information.creation_date;
          var plant = sqmsver_shipping_information.plant;
          var order_number = sqmsver_shipping_information.order_number;
          var supplier = sqmsver_shipping_information.supplier;

          var sqmservercontactPerson = sqmserver.contact_person;
          var cancellation_flag = sqmsver_shipping_information.cancellation_flag;
          var receiverAddress = sqmsver_shipping_information.receiverAddress;
          ...
          for (var i = 0; i < shippingpartnum.length; i++) {
            if (i <= 13) {
              partnumberdata.push(shippingpartnum[i]);
            }
            if (i >= 15 && i <= 18) {
              es1.push(shippingpartnum[i]);
            }
            if (i >= 20 && i <= 23) {
              es2.push(shippingpartnum[i]);
            }
          }
